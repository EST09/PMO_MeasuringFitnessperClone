...
print(len(nucl["present"] == 1))
nuc_coords = coord[coord["nucleus"] == 1]
nuc_coords = nuc_coords[["x", "y", "z"]]
mito_coords = coord[coord["mitochondrion"] == 1]
mito_coords = mito_coords[["x", "y", "z"]]
...
print(nuc_coords[(nuc_coords['x'] == a[0])&(nuc_coords['y'] == a[1])&(nuc_coords['z'] == a[2])].index[0])

for i, j in zip(pathway_n, expr_n):  
    
    tree = KDTree(n_array)
    nearest_dist, nearest_ind = tree.query([n_array[0]], k=j)    

    coloured_array = np.take(n_array, nearest_ind[0], 0)
    
    indices = []
    for c in coloured_array: 
       index =  nuc_coords[(nuc_coords['x'] == c[0])&(nuc_coords['y'] == c[1])&(nuc_coords['z'] == c[2])].index[0]
       indices.append(index) 
    
    n_array = np.delete(n_array, nearest_ind[0], 0)

    nuc_coords.loc[indices,'colour'] = i
